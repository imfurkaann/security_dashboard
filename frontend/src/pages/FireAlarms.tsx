...
export default function FireAlarms() {
    ...
    }, [alarmNumber, location, alarmTime, resolutionTime, resolutionNotes, falseAlarm]);
    ...
    const isEditFormDirty = useMemo(() => {
        ...
        return (
            alarmNumber !== (originalRecord.alarm_number || '') ||
            location !== originalRecord.location ||
            alarmTime !== (originalRecord.alarm_time ? formatTime(originalRecord.alarm_time) : '') ||
            resolutionTime !== (originalRecord.resolution_time ? formatTime(originalRecord.resolution_time) : '') ||
            resolutionNotes !== (originalRecord.resolution_notes || '') ||
            falseAlarm !== originalRecord.false_alarm
        );
    }, [alarmNumber, location, alarmTime, resolutionTime, resolutionNotes, falseAlarm, editingId, records]);
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
                                ...
                                <div className="min-h-full">
                                    <table className="w-full min-w-[1000px] table-auto divide-y divide-gray-200">
                                        <thead className="bg-gray-50">
                                            <tr>
                                                <th className="w-[240px] px-3 py-2.5 whitespace-nowrap text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">İşlemler</th>
                                                ...
                                                <th className="w-[110px] px-3 py-2.5 whitespace-nowrap text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Kaydeden</th>
                                            </tr>
                                        </thead>
                                            ...
                                            {filteredRecords.map(record => (
                                                <tr key={record.id} className={`hover:bg-gray-50 ${record.deleted_at ? 'opacity-60' : ''}`}>
                                                        ...
                                                        <div className="flex items-center gap-2 whitespace-nowrap">
                                                                ...
                                                                {record.deleted_at ? (
                                                                    ...
                                                                    <>
                                                                        {record.resolved && (
                                                                            <CompactActionButton
                                                                                onClick={() => handleUndoResolve(record.id)}
                                                                                variant="success"
                                                                                label="Geri Al"
                                                                                icon={
                                                                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <td className="px-3 py-2.5 align-top">
                                                        <div className="text-xs font-medium text-gray-900 break-words">{record.alarm_number || '-'}</div>
                                                    </td>
                                                    <td className="px-3 py-2.5 align-top">
                                                    ...
                                                    <td className="px-3 py-2.5 align-top">
                                                        ...
                                                        <div className="text-[10px] text-gray-600">{formatTime(record.alarm_time)}</div>
                                                    </td>
                                                    <td className="px-3 py-2.5 align-top">
                                                        {record.resolution_time ? (
                                                            <>
                                                                <div className="text-xs text-gray-900">{formatDate(record.resolution_time)}</div>
                                                                <div className="text-[10px] text-gray-600">{formatTime(record.resolution_time)}</div>
                                                            </>
                                                        ) : (
                                                    ...
                                                    <td className="px-3 py-2.5 align-top whitespace-nowrap">
                                                        {record.resolved ? (
                                                            <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-green-100 text-green-800">Çözüldü</span>
                                                        ) : (
                                                            <span className="px-2 py-0.5 text-[10px] font-medium rounded-full bg-red-100 text-red-800">Aktif</span>
                                                        )}
                                                    </td>
                                                    <td className="px-3 py-2.5 align-top">
                                                        <div className="text-xs text-gray-900 break-words">{record.recorded_by_name || '-'}</div>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                    ...
                    <div className="flex flex-col gap-3">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
            ...
            <CustomModal
                ...
                size="md"
                closeOnBackdropClick={false}
                hasUnsavedChanges={resolutionNotes !== '' || falseAlarm !== false}
            >
                <div className="space-y-3">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Çözüm Notları</label>
                        <textarea
                            value={resolutionNotes}
                            onChange={(e) => setResolutionNotes(e.target.value)}
